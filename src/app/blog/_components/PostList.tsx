		...
		<main className='container mx-auto max-w-screen-md'>
			{posts.map((post) => (
				...
				>
					<div className='hidden sm:block'>
						{post?.mainImage ? (
							<Link href={viewPath + post.slug.current} className='block relative m-2'>
								<div className='flex items-center justify-center min-h-[150px] min-w-[150px] overflow-hidden border-[3px] border-dark'>
									<Image
										className=''
					...
					<div className=''>
						<p className='absolute top-0 right-0 bg-dark p-2 m-2 text-sm leading-3 text-gray-200 z-10'>
							{post.series ? "Series: " : ""}
						...
						<div className='flex h-full items-center relative p-2'>
							<Link key={post._id} href={viewPath + post.slug.current} className='my-10'>
								<h2 className='bg-dark px-2 py-4 text-xl sm:text-2xl text-white hover:text-primary'>
									{post.title}
								</h2>
							</Link>
						</div>

						<p className='absolute bottom-0 right-0 bg-dark  p-2 m-2 text-sm leading-3 text-gray-200 z-10'>
